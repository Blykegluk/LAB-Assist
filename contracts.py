...
def create_contract(payload: ContractCreate, db: Session = Depends(get_db)):
    ...
    # Si pdf_path est déjà une URL (ex: Supabase Storage), on la renvoie telle quelle
...
def list_contracts(
    ...
    for c in rows:
        ...
        if c.generated_doc_path:
            if isinstance(c.generated_doc_path, str) and c.generated_doc_path.lower().startswith("http"):
                url = c.generated_doc_path
            else:
                from pathlib import Path
                gen_dir = ensure_generated_dir()
                try:
