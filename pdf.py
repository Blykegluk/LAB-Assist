...
def generate_contract_pdf(contract: dict) -> str:
    ...
    try:
        local_path = _generate_pdf_from_text_template(txt_template, variables, out_path)
    except Exception as e:
        raise RuntimeError(f"Erreur lors de la génération du PDF: {str(e)}") from e

    # Upload optionnel vers Supabase Storage (si configuré), pour rendre le lien durable
    try:
        sb = SupabaseStorageClient()
        if sb.is_configured():
            # Dossier logique par magasin pour lisibilité
            store = (contract.get("store") or "STORE").upper()
            object_name = f"{store}/{filename}"
            public_url = sb.upload_file(Path(local_path), object_name=object_name, content_type="application/pdf")
            return public_url  # Retourne directement l'URL publique
    except Exception as e:
        # En cas d'échec de l'upload, on conserve le chemin local (fallback)
        pass
